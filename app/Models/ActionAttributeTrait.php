...
trait ActionAttributeTrait{
	...
	public function getShowActionButton($type = true)
	{
		...
		//开启查看按钮
		if (config('admin.globals.'.$this->action.'.show')) {

			if (Auth::user()->can(config('admin.permissions.'.$this->action.'.show'))) {

				if($this->action =='game'){
					if ($type) {
						return '<a href="'.url('admin/basedata/all'.'?appid='.$this->appid).'" class="btn btn-xs btn-info tooltips" data-container="body" data-original-title="' . trans('labels.'.$this->action.'.show') . '"  data-placement="top"><i class="fa fa-search"></i></a>';
					}
					return '<a href="'.url('admin/basedata/all'.'?appid='.$this->appid).'" class="btn btn-xs btn-info tooltips" data-toggle="modal" data-target="#draggable" data-container="body" data-original-title="' . trans('labels.'.$this->action.'.show') . '"  data-placement="top"><i class="fa fa-search"></i></a>';
				}
				if ($type) {
	...
	{
		//dd($this->action);
		//dd(config('admin.permissions.'.$this->action.'.edit'));
//		dd(auth()->user()->hasRole('admin'));
//		dd(Auth::user()->can(config('admin.permissions.'.$this->action.'.edit')));
		if (Auth::user()->can(config('admin.permissions.'.$this->action.'.edit'))) {
			return '<a href="'.url('admin/'.$this->action.'/'.$this->id.'/edit').'" class="btn btn-xs btn-primary tooltips" title="' . trans('crud.edit') . '" data-original-title="' . trans('crud.edit') . '"  data-placement="top"><i class="fa fa-pencil"></i></a>';
		}
		return '';
	}
	...
	{
		if (($this->status == config('admin.globals.status.active')) || ($this->status == config('admin.global.status.audit'))) {
	...
	{
		if (($this->status == config('admin.globals.status.destory')) || ($this->status == config('admin.global.status.trash'))) {
	...
	{
		if (($this->status == config('admin.globals.status.audit'))) {
